...
############################ Read Input Data #######################################
# Read in Cell Calls for infected and mock samples
# "mm10" = mouse cell
Infected <- read.csv("gem_classification.csv")
Infected_mouse <- subset(Infected, Infected$call=="mm10")

NOTinfected <- read.csv("gem_classification.csv")
NOTinfected_mouse <- subset(NOTinfected, NOTinfected$call=="mm10")

################## read in sample 1 (Infected)
INF = Read10X("filtered_feature_bc_matrix/")

# remove mouse cells (INF)  
INF <- INF[,!colnames(INF)%in%Infected_mouse$barcode]

# Create Sample1 Seurat Object (Infected)
infected <- CreateSeuratObject(counts = INF, project = "infected", min.cells = 3, min.features = 200)

################# read sample 2 (Control)
NI = Read10X("filtered_feature_bc_matrix/")

# remove mouse cells (NI) 
NI <- NI[,!colnames(NI)%in%NOTinfected_mouse$barcode]

# create sample2 suerat object
NotInf <- CreateSeuratObject(counts = NI, project = "NotInf", min.cells = 3, min.features = 200)

############################################# Filtering ###########################
# Identify Condition
infected[["sample"]] <- "Infected"
NotInf[["sample"]] <- "Mock"

# Merge Samples
merged_seurat <- merge(x=infected, y=NotInf, add.cell.id = c("Infected", "Mock"))

# calculate the proportion of transcripts mapping to mitochondrial genes (gene names begin with "MT")
merged_seurat[["percent.mt"]] <- PercentageFeatureSet(merged_seurat, pattern = "MT-")

# Plot Quality - # Genes/Cell, RNA-Counts/Cell, Mitochondrial Gene Percentage
...
Filter <- subset(x=merged_seurat, subset=nFeature_RNA > 200 & nFeature_RNA < 9000 & percent.mt > -Inf & percent.mt < 6)
# Reassess Plots
VlnPlot(Filter, features=c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol=3)
FeatureScatter(object=Filter, feature1="nCount_RNA", feature2= "percent.mt", cols=c("red", "blue"))
FeatureScatter(object=merged_seurat, feature1="nCount_RNA", feature2= "nFeature_RNA", cols=c("red", "blue"))

# Split Merged Samples
split_seurat <- SplitObject(Filter, split.by = "sample")
split_seurat <- split_seurat[c("Infected", "Mock")]

############################################################## DOUBLET FINDER #######
# Pre-Process Seurat Object
# SCTransform performs Normalization and Variance Stabilization
# Run Principal Component Analysis and render UMAP
...
###################### Establish Function to Find Doublets
findDoublets <- function(seu, cluster) {
  # seu = seurat object
  # cluster = seurat metadata column containing clusters
  
  dfr_table <- data.frame(num_cells = c(500, seq(1000, 10000, 1000)),
                          multiplet_rate = c(0.004, 0.008, 0.016, 0.023, 
                                             0.031, 0.039, 0.046, 0.054, 0.061, 0.069, 0.076))
  
  row <- nrow(dfr_table %>%  filter(num_cells <= length(Cells(seu))))
  dfr <- (((cell_count - dfr_table$num_cells[row])/1000) * (dfr_table$multiplet_rate[row+1] -
                                                              dfr_table$multiplet_rate[row])) + dfr_table$multiplet_rate[row]
  
  sweep.res.list_1 <- paramSweep(seu, PCs= 1:10, sct=T)
...
findDoublets(split_seurat[[1]], "SCT_snn_res.0.6")
findDoublets(split_seurat[[2]], "SCT_snn_res.0.6")


# REMOVE DOUBLETS #
split_seurat[[1]] <- subset(x = split_seurat[[1]], subset = DF.classifications_0.25_0.15_197 !="Doublet") 
split_seurat[[2]] <- subset(x = split_seurat[[2]], subset = DF.classifications_0.25_0.29_359 !="Doublet") 


########################################################## Downsample ###########
set.seed(111)
# Down-sample cell-sets to smallest sample size for even comparison
split_seurat[[1]] <- split_seurat[[1]][, sample(colnames(split_seurat[[1]]), size = min(c(length(Cells(split_seurat[[1]])), length(Cells(split_seurat[[2]])))), replace=F)] 
split_seurat[[2]] <- split_seurat[[2]][, sample(colnames(split_seurat[[2]]), size = min(c(length(Cells(split_seurat[[1]])), length(Cells(split_seurat[[2]])))), replace=F)]
...
########################### Manual Annotation Bone Marrow Sample ####################################
#### Feature Plots
DefaultAssay(combined.all) <- "integrated"
# Feature Plots
DefaultAssay(combined.all) <- "RNA"
...
deseq <- function(seu, x) {
  ...
  write.csv(pval, paste('/scRNA_BM_sara/DEGs', x, 'csv', sep = '.'))
}
...
# Run Find All Markers on All Annotated Clusters for Infected vs. Mock
# Default : Wilcoxon Rank Sum Test
# Create new Metadata column detailing cluster + infection status
...
cluster_names <- unique(combined.all$celltype.stim)

# Loop through clusters and run FindMarkers() for each and save table for each
for (i in 1:length(cluster_names)) {
  uninf <- paste(cluster_names[i], "Not", sep = "_")
  inf <- paste(cluster_names[i], "Infected", sep = "_")
  temp <- FindMarkers(combined.all, ident.1 = inf, ident.2 = uninf, verbose = FALSE)
  new_name <- paste(i, "DEgenes", sep = "_")
...
######################## Trajectory Analysis ####################################
## DE TRAJECTORY 
library(slingshot); library(SingleCer 10llExperiment)
...
# Run Slingshot Trajectory Analysis - different options
...
# FINAL VISUALIZATIONS for Bone Marrow scRNA HIV+ vs. Mock############################################################ 
